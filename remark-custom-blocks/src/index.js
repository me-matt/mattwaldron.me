...
module.exports = function blockPlugin (availableBlocks = {}) {
  ...
  function blockTokenizer (eat, value, silent) {
    ...
    let potentialTitle = potentialBlock.title
    let potentialBlockTitle = blockTitle

    if (!blockTitle && potentialTitle) {
      if (!['optional', 'required'].includes(potentialTitle)) {
        potentialBlockTitle = potentialTitle
        potentialTitle = 'required'
      }
    }

    const titleAllowed = potentialTitle &&
      ['optional', 'required'].includes(potentialTitle)
    const titleRequired = potentialTitle && potentialTitle === 'required'

    if (titleRequired && !potentialBlockTitle) return
    if (!titleAllowed && potentialBlockTitle) return
    ...
    if (titleAllowed && potentialBlockTitle) {
      ...
      const titleNode = {
        ...
        children: this.tokenizeInline(potentialBlockTitle, now),
      }
